...
- **Python-Laufzeit.** In Python geschriebene Plugins benötigen einen Interpreter; die mitgelieferten Plugins sind solche Plugins. Beim ersten Start eines solchen Plugins lädt das mit smabar ausgelieferte Werkzeug uv einmalig eine CPython-3.14-Distribution aus den python-build-standalone-Releases auf GitHub (github.com, objects.githubusercontent.com) und speichert sie unter `~/.smabar/tools/`. GitHub erhält dabei Ihre IP-Adresse wie oben beschrieben. Systemweit wird nichts installiert.
